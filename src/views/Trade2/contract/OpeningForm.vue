...
<template>
  <div class="form">
    ...
    <div class="item_box" style="overflow: hidden; border-radius: 0 0.32rem 0 0">
      <div class="tip-box" @click="tipZ" :style="{ opacity: volumeF ? '0' : '1' }">
    ...
    <Button v-if="token" :loading="configLoading || submitLoading" size="large" @click="submit1"
      class="submit ripple-btn" :color="activeType == 1 ? 'var(--ex-primary-color)' : 'var(--ex-down-color)'
        " round>
  ...
  <BottomPopup teleport="body" v-model:show="showModel" position="bottom" round closeable>
    ...
    <div class="stock_submit_box">
      ...
      <div class="item pass_ipt">
        <input style="width: 100%; height: 100%" v-model="safePass" :placeholder="t('trade.stock_opening_trade_pw')"
...
const tipZ = () => {
  showToast(
    `1${t('trade.contract_one_lot')} = ${amountper.value} ${paramCurrency.value
    }`,
  );
};
