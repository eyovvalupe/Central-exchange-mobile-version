...
<template>
  <div class="page page-loginpass">
    ...
    <div class="form">
      ...
      <div class="form_item margin_item transition" :style="{ borderColor: newError ? 'var(--ex-error-color)' : '' }">
        <input maxlength="20" v-model.trim="form.password" :type="showPass ? 'text' : 'password'"
          :placeholder="$t('change_login_pw.new_pw')" class="item_input" @focus="newError = false" />
      ...
      <div class="form_item margin_item transition"
        ...
        <input maxlength="20" v-model.trim="form.password2" :type="showPass2 ? 'text' : 'password'" class="item_input"
          :placeholder="$t('change_login_pw.confirm_pw')" @focus="confirmError = false" />
...
<script setup>
import { Button, showToast, Loading } from 'vant';
...
const submitForm = () => {
  const params = {
    oldpassword:form.value.prevPassword,
